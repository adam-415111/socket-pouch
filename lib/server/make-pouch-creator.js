...
var PouchDB = require('pouchdb-core')
  .plugin(require('pouchdb-adapter-node-memory'))
var Promise = require('pouchdb-adapter-memory');
...
function createLocalPouch(args) {
  ...
  return Promise.resolve({
    pouch: new PouchDB(args[0], { adapter: 'memory' })
  });
}
...
function createHttpPouch(options) {
  ...
  return function (args) {
    ...
    return Promise.resolve({
      pouch: new PouchDB(remoteUrl + '/' + args[0].name, { adapter: 'memory' })
    });
  };
}
