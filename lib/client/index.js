...
function SocketPouch(opts, callback) {
  ...
  function createNewSocket(sock) {
    ...
    function receiveMessage(res, topic) {
    ...
    if (true) {
      log('listening for events from ' + api._name)
      api._socket.on(api._name, function (res) {
        var topic = '' + api._name
        if (typeof res !== 'string') {
          return receiveBlob(res, topic);
        }
        receiveMessage(res, topic);
      });
      // for createDatabase..:
      api._socket.on('pouch', function (res) {
        // if (typeof res !== 'string') {
        //   return receiveBlob(res);
        // }
        receiveMessage(res, 'pouch');
      });
  ...
  function sendMessage(type, args, callback) {
    ...
    if (true) {
      var topic = (type === 'createDatabase') ? 'pouch' : api._name
      api._socket.emit(topic, type + ':' + messageId + ':' + stringArgs, function () {
  ...
  function sendBinaryMessage(type, args, blobIndex, blob, callback) {
    ...
    if (true) {
      api._socket.emit(api._name, blobToSend, function () {
  ...
  api._changes = function (opts) {
    ...
    if (opts.continuous) {
      ...
      if (true) {
        api._socket.emit(api._name, 'liveChanges' + ':' + messageId + ':' + JSON.stringify([opts]));
        return {
          cancel: function () {
            api._socket.emit(api._name, 'cancelChanges' + ':' + messageId + ':' + JSON.stringify([]));
          }
        };
      } else {
